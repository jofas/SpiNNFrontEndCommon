...
static inline int _find_n_available_mallocs(heap_t *sdram_heap) {
...
void _build_malloc_tracker(void) {
    // malloc tracker
...
static inline uint _find_free_space_available(
...
static inline void _make_heap_structure(
...
void _print_free_sizes_in_heap(void) {
...
bool malloc_extras_initialise_fake_heap(
    ...
    uint n_mallocs = _find_n_available_mallocs(sv->sdram_heap);
    sdram_block *list_of_available_blocks =
    ...
    stolen_sdram_heap->free_bytes = _find_free_space_available(sizes_region);

    // go through true heap and allocate and add to end of list.
    bool success = _add_heap_to_collection(list_of_available_blocks);
    if (!success) {
    ...
    _make_heap_structure(
        sizes_region, n_mallocs, list_of_available_blocks);
...
bool malloc_extras_initialise_no_fake_heap_data(void) {
    return malloc_extras_initialise_fake_heap(NULL);
}
...
void malloc_extras_safe_x_free_marked(void *ptr, int marker) {
    ...
    if (safety) {
        ...
        // print which index is being freed, incase
        if(to_print) {
            bool found = false;
            for (int index = 0; index < malloc_points_size; index ++) {
...
void malloc_extras_safe_x_free(void *ptr) {
    malloc_extras_safe_x_free_marked(ptr, -1);
}

//! \brief doubles the size of the SDRAM malloc tracker
void _build_bigger_malloc_tracker(void) {
...
//! pointer.
int _find_free_malloc_index(void) {
    int index;
...
void * _safe_sdram_malloc(uint bytes) {
