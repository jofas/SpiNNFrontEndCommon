...
def run(hostname, bmp_details, downed_chips, downed_cores,
    ...
    if scamp_connection_data is not None:
        scamp_connection_data = _sort_out_scamp_connections(
            scamp_connection_data)

    # sort out BMP connections into list of strings
    bmp_connection_data = _sort_out_bmp_string(bmp_details)
...
def _sort_out_bmp_string(bmp_string):
    ...
    for bmp_detail in bmp_string.split(":"):
        ...
        (cabinet, frame, hostname, port_num) = \
            _sort_out_bmp_cabinet_and_frame_string(
                bmp_string_split[0])
        ...
        if len(bmp_string_split) == 1:
            ...
            boards = _sort_out_bmp_boards_string(bmp_string_split[1])
            if port_num is not None:
                bmp_details.append(
...
class MachineGenerator(object):
    ...
    def __call__(
        ...
        """
        run(hostname, bmp_details, downed_chips, downed_cores, downed_links,
